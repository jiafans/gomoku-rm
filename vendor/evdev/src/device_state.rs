...
impl DeviceState {
    ...
    pub(crate) fn process_event(&mut self, ev: InputEvent) {
        // Patched for reMarkable 2: the rM2 multitouch (`pt_mt`) device emits
        // BTN_TOUCH key events even though its EVIOCGBIT(EV_KEY,..) response
        // declares no key support. Original evdev `.expect()`s on this and
        // panics the input thread, hanging the app. We silently skip events
        // when the corresponding state cache wasn't allocated.
        match ev.kind() {
            InputEventKind::Key(code) => {
                if let Some(keys) = self.key_vals.as_deref_mut() {
                    keys.set(code, ev.value() != 0);
                }
            }
            InputEventKind::AbsAxis(axis) => {
                if let Some(axes) = self.abs_vals.as_deref_mut() {
