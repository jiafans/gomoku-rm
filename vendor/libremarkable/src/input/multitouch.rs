        ...
        ecodes::EV_ABS => {
            ...
            match ev.code() {
                ...
                ecodes::ABS_MT_TRACKING_ID => match ev.value() {
                    ...
                    _ => {
                        // Patched for reMarkable 2: the touch driver does not
                        // emit ABS_MT_PRESSURE (`min=0 max=0`), so the original
                        // handler — which only sets `pressed = true` from
                        // ABS_MT_PRESSURE > 0 — never produced Press events.
                        // A non-negative tracking_id is the canonical MT-B
                        // signal that a slot is active, so we treat that as
                        // pressed. Release is still driven by tracking_id == -1.
                        let finger = fingers.entry(current_slot).or_default();
                        finger.tracking_id = ev.value();
                        finger.pressed = true;
                        vec![]
                    }
                },
